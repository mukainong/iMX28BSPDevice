...
UINT32 SGTL5000Codec::SGTL5000I2CInit(VOID)
{
    ...
    m_hI2C = I2COpenHandle(SGTL5000_I2C_PORT);
    ...
    if (!I2CSetClockRate(m_hI2C, dwFrequency))
    {
        uiStatus = SGTL5000_I2C_OPERROR;
        SGTL5000I2CClose();
        return uiStatus;
...
{    
    ...
    data[0] = SGTL5000_I2C_ADDR | 0;
    data[1] = (UINT8)(( usRegNum >> 8 ) & 0xFF);
    data[2] = (UINT8)(( usRegNum ) & 0xFF);
    data[3] = (UINT8)(( usValue >> 8 ) & 0xFF);
    data[4] = (UINT8)(( usValue ) & 0xFF);
    ...
    if (TRUE != uiStatus)
    {
        ERRORMSG(TRUE, (_T("SGTL5000WriteRegister(0x%x)=0x%x failed!\r\n"), 
         usRegNum, usValue));            
    }
...
UINT32 SGTL5000Codec::SGTL5000ReadRegister( UINT16 usRegNum, UINT16* pusValue )
{
    ...
    uiRegData[0] = SGTL5000_I2C_ADDR | 0;
    uiRegData[1] = (UINT8)(( usRegNum & 0xFF00) >> 8);
    uiRegData[2] = (UINT8)(usRegNum & 0x00FF);

    I2CPktArray[0].pbyBuf = uiRegData;
    I2CPktArray[0].wLen = 3; // let I2C know how much to transmit
    I2CPktArray[0].byRW = I2C_RW_WRITE;
    I2CPktArray[0].lpiResult = &iResult; 

    uiRegValue[0] = SGTL5000_I2C_ADDR | 1;
    ...
    I2CPktArray[2].pbyBuf = uiRegValue;
    I2CPktArray[2].wLen = 2; // let I2C know how much to receive
    ...
    *pusValue = ((UINT16)uiRegValue[0] << 8) | (UINT16)uiRegValue[1];
    ...
    if (TRUE != uiStatus)
    {
        ERRORMSG(TRUE, (_T("SGTL5000ReadRegister(0x%x)=0x%x failed!\r\n"), 
            usRegNum, *pusValue));
    }
...
UINT32 SGTL5000Codec::SGTL5000CodecInit(VOID)
{
    ...
    // to 75mA ~ 100mA. Note:Capless mode is enabled
    RegValue = CSP_BITFVAL(MODE_CM, MODE_CM_ENABLE_DETECTOR_RESET_TIMEOUT) |
               CSP_BITFVAL(LVLADJR, LVLADJC_100MA) |
    ...
    {
        SYSMCLK /= 2;
        RegValue = CSP_BITFVAL(INPUT_FREQ_DIV2, INPUT_FREQ_DIV2_DIV2);
        RegMask = CSP_BITFMASK(INPUT_FREQ_DIV2);   
        SGTL5000ModifyRegister(SGTL5000_CHIP_CLK_TOP_CTRL_ADDR, RegMask, RegValue);
    }

    PllOutputFreq = PLL_OUTPUT_FREQ_180633600HZ;
    Int_Divisor = (UINT16)(PllOutputFreq/SYSMCLK);
    Frac_Divisor = (UINT16)((((float)PllOutputFreq/(float)SYSMCLK)-Int_Divisor)*2048);
    ...
    SGTL5000ModifyRegister(SGTL5000_CHIP_ANA_ADC_CTRL_ADDR, RegMask, RegValue);

    // Increase PAD strength
    //SGTL5000WriteRegister( SGTL5000_CHIP_PAD_STRENGTH_ADDR, 0xFFFF); 
...
VOID SGTL5000Codec::SGTL5000ConfigurePowerMode(UINT16 usPowerMode)
{
    ...
    switch (usPowerMode)
    {
        case POWER_MODE_CODEC_OFF:
            ...
            //RegValue1 =  CSP_BITFVAL(VCOAMP_POWERUP, ENABLE) | 
            //            CSP_BITFVAL(LINREG_D_POWERUP, ENABLE)|
            //            CSP_BITFVAL(REFTOP_POWERUP, ENABLE) |
            //            CSP_BITFVAL(PLL_POWERUP, ENABLE);

            RegValue1 |= CSP_BITFVAL(VCOAMP_POWERUP, DISABLE)    |
                         CSP_BITFVAL(DAC_POWERUP, DISABLE)       |
                         CSP_BITFVAL(CAPLESS_HP_POWERUP, DISABLE)|
                         CSP_BITFVAL(REFTOP_POWERUP, DISABLE)    |
                         CSP_BITFVAL(RIGHT_DAC_POWERUP, DISABLE) |
                         CSP_BITFVAL(LINEOUT_POWERUP, DISABLE)   |
                         CSP_BITFVAL(HEADPHONE_POWERUP, DISABLE) |
                         CSP_BITFVAL(PLL_POWERUP, DISABLE);


            SGTL5000WriteRegister(SGTL5000_CHIP_ANA_POWER_ADDR, RegValue1);
            //SGTL5000WriteRegister(SGTL5000_CHIP_ANA_POWER_ADDR, 0x2548);
        ...
        case POWER_MODE_CODEC_ON:
            ...
            RegValue1 |= CSP_BITFVAL(DAC_POWERUP, ENABLE)       |
                         CSP_BITFVAL(CAPLESS_HP_POWERUP, ENABLE)|
                         CSP_BITFVAL(REFTOP_POWERUP, ENABLE)    |
            ...
            RegValue = CSP_BITFVAL(SYS_FS, SYS_FS_44_1_KHZ) |
                       CSP_BITFVAL(MCLK_FREQ, MCLK_FREQ_512_FS/*MCLK_FREQ_PLL*/);
            ...
            {
                case 16:
                    // Set as I2S slave, 16-bit data length
                    RegValue = CSP_BITFVAL(DLEN, DLEN_16_BITS) |
                               CSP_BITFVAL(SCLKFREQ, SCLKFREQ_32_FS); 
                    break;
                case 24:
                    // Set as I2S slave, 24-bit data length
                    RegValue = CSP_BITFVAL(DLEN, DLEN_24_BITS) |
                               CSP_BITFVAL(SCLKFREQ, SCLKFREQ_32_FS);
                    break;
                default:
                    ERRORMSG(TRUE,(TEXT("Error,not supported sample rate\r\n")));
                    return;
                    break;
            }
            ...
            if(m_workMode == WORK_MODE_LOOPBACK){
                ...
                SGTL5000ModifyRegister(SGTL5000_CHIP_DIG_POWER_ADDR, RegMask, RegValue); 
        ...
        case POWER_MODE_RECORD_ON:
            ...
            // Power up the ADC
            RegValue = CSP_BITFVAL(ADC_POWERUP, ENABLE) | CSP_BITFVAL(RIGHT_ADC_POWERUP, ENABLE);
            RegMask = CSP_BITFMASK(ADC_POWERUP) | CSP_BITFMASK(RIGHT_ADC_POWERUP);            

            SGTL5000ModifyRegister(SGTL5000_CHIP_ANA_POWER_ADDR, RegMask, RegValue);
        ...
        case POWER_MODE_RECORD_STOP:
            ...
            // Power down ADC.
            RegValue = CSP_BITFVAL(ADC_POWERUP, DISABLE) | CSP_BITFVAL(RIGHT_ADC_POWERUP, DISABLE);
            RegMask = CSP_BITFMASK(ADC_POWERUP) | CSP_BITFMASK(RIGHT_ADC_POWERUP); 
...
VOID SGTL5000Codec::SGTL5000RampHPVolume(UINT16 usNewVol)
{
    ...
    //SGTL5000WriteRegister(SGTL5000_CHIP_ANA_HP_CTRL_ADDR, usNewVol);
    //m_usHPVol = usNewVol;
...
VOID SGTL5000Codec::SGTL5000SetDACPower(UINT16 usPower)
{
    ...
    RegValue = CSP_BITFVAL(DAC_POWERUP, usPower) | CSP_BITFVAL(CAPLESS_HP_POWERUP, usPower);
    RegMask = CSP_BITFMASK(DAC_POWERUP) | CSP_BITFMASK(CAPLESS_HP_POWERUP);
    SGTL5000ModifyRegister(SGTL5000_CHIP_ANA_POWER_ADDR, RegMask, RegValue);
}
